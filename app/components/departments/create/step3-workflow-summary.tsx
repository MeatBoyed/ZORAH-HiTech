...
export default function Step3WorkflowSummary({
  ...
  const [vapiWorkflow, setVapiWorkflow] = useState<object | null>(null);
  const [finalDepartmentObject, setFinalDepartmentObject] = useState<object | null>(null);

  useEffect(() => {
    // Generate VAPI workflow and final department object
    const enabledStaff = (formData.staff || []).filter((s: Staff) => s.callConfig?.enabled);

    if (enabledStaff.length > 0 && formData.department) {
      try {
        // Create a temporary Department object for preview
        ...
        const finalObject = generateFinalDepartmentObject(
          tempDepartment,
          formData.staff || [],
          formData.workflow || { name: "", description: "", baseType: "daily-check-in" as const },
          formData.schedule || { cron: "0 8 * * 1,2,3,4,5", timeZone: "Africa/Johannesburg", enabled: true }
        );
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
            ...
            <div className="text-center">
              <div className="text-2xl font-bold text-orange-600">{enabledStaff.filter((s: Staff) => s.callConfig?.escalationEnabled).length}</div>
              <div className="text-sm text-gray-500">With Escalation</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-purple-600">{enabledStaff.reduce((sum: number, s: Staff) => sum + (s.callConfig?.captureFields?.length || 0), 0)}</div>
          ...
          {enabledStaff.length > 0 && (
            <div className="space-y-3">
              ...
              {enabledStaff.map((staff: Staff, index: number) => (
                <div key={staff.id} className="border rounded-lg p-3 bg-gray-50">
                  ...
                  <div className="text-sm text-gray-600">
                    ...
                    {(staff.callConfig?.captureFields?.length || 0) > 0 && (
                      <span className="ml-2">
                        ({staff.callConfig?.captureFields?.map((f: { name: string }) => f.name).join(", ")})
                      </span>
                    )}
                  </div>
                </div>
